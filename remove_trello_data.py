...
def prepareForArchive(card, daysSince):
    return filterByTime(card, getPrepareArchiveDate(daysSince)) and filterByState(
        card, ("closed", False)
    )


def prepareForDelete(card, daysSince):
    return filterByTime(card, getPrepareDeleteDate(daysSince)) and filterByState(
        card, ("closed", True)
    )
...
def performArchiveAndDelete(action, daysSince):
    ...
    if action == "archive":
        ...
        toArchiveList = list(
            filter(lambda card: prepareForArchive(card, daysSince), cards)
        )
        ...
        toDeleteList = list(
            filter(lambda card: prepareForDelete(card, daysSince), cards)
        )
