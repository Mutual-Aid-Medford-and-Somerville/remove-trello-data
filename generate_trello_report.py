...
def generateReport(cardInfo, reportType):
    ...
    if reportType == "Archive":
        mdFile.write(
            "The following is a list of cards that will be Archived a week after the"
            f" creation of this report (on {datestr}). "
        )
        mdFile.write(
            "Archived Trello cards are still accessible within the application, but"
            " they are hidden from normal view. "
        )
        mdFile.write(
            "Archival means that these cards will be deleted from Trello next month if"
            " nothing changes, in accordance with the tech-team data privacy policy. "
        )
        mdFile.write(
            "If you wish to preserve these cards going forward, please update them in"
            " the Trello. If updated, the card will no longer be slated for archival"
            " during the upcoming Archive run. "
        )
        mdFile.write(
            "Please reach out to the tech-team if you have any questions about this"
            " policy or Trello Archival."
        )
    elif reportType == "Delete":
        mdFile.write(
            "The following is a list of cards that will be Deleted a week after the"
            f" creation of this report (on {datestr}). "
        )
        mdFile.write(
            "Deleted Trello cards are removed from the service entirely, and any"
            " associated data will be gone. "
        )
        mdFile.write(
            "As per the tech-team privacy policy, we only will delete Archived cards"
            " that have not been recently updated. "
        )
        mdFile.write(
            "Therefore, if you wish to preserve this card, please un-archive it. "
        )
        mdFile.write(
            "However, please consider allowing it to be deleted. We would prefer to"
            " keep as little data on our community members as possible. "
        )
        mdFile.write(
            "Cards that have not been updated a week after this report is published"
            " will be deleted. "
        )
    else:
    ...
    mdFile.write(
        "Please reach out to the tech-team if you have any questions about the privacy"
        " policy, these reports, or our Trello clean-up actions."
    )
    mdFile.new_line()
    mdFile.new_line()
    ...
    mdFile.write(
        f"## The following {len(cardInfo)} cards will be {upcomingAction} next week"
    )
    mdFile.new_line()
    mdFile.new_line()
